...
def load_image(image_file, input_size=448, max_num=12):
    if isinstance(image_file, str):
        ...
        if image_file.shape[-1] == 1:
            image_file = np.squeeze(image_file)
            image = Image.fromarray(image_file, mode="L").convert("RGB")
        elif image_file.shape[-1] == 3:
            image = Image.fromarray(image_file, mode="RGB").convert("RGB")
